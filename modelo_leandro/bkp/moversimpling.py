...
# Aplicar oversampling nos dados desbalanceados
ros = RandomOverSampler(random_state=42)
X = df['textfull']
y = df['classificado']
X_resampled, y_resampled = ros.fit_resample(X.values.reshape(-1, 1), y)

# Converter os dados de volta para DataFrame
df_resampled = pd.DataFrame(X_resampled, columns=['textfull'])
df_resampled['classificado'] = y_resampled

# Dividir em conjunto de treino e teste
X_train, X_test, y_train, y_test = train_test_split(df_resampled['textfull'], df_resampled['classificado'], test_size=.30, random_state=random.randint(1, 100), shuffle=True)
