...
# Mapeamento inverso para rótulos
...
# Contagem de amostras da classe 'Concluso'
concluso_count = df[df['classificado'] == 'Concluso'].shape[0]

# Se a contagem for menor que 2500, manter todas as amostras de 'Concluso'
if concluso_count < 3000:
    concluso_subset = df[df['classificado'] == 'Concluso']
else:
    concluso_subset = df[df['classificado'] == 'Concluso'].sample(n=3000, random_state=42)

other_records = df[df['classificado'] != 'Concluso']
...
# Converter os valores previstos de volta para os rótulos originais
predicted_labels = [mapeamento_inverso[pred] for pred in predictions]
y_test_labels = [mapeamento_inverso[val] for val in y_test]

# Calcular a acurácia do modelo
accuracy = accuracy_score(y_test_labels, predicted_labels)
print(f'Acurácia do modelo: {accuracy}')

# Calcular e imprimir a matriz de confusão com os rótulos
conf_matrix = confusion_matrix(y_test_labels, predicted_labels, labels=list(mapeamento_inverso.values()))
